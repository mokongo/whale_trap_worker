...
CHAT_ID = os.getenv("TELEGRAM_CHAT_ID", "5929815952")  # fallback to @mokongo ID
...
def send_telegram_alert(message):
    if TELEGRAM_TOKEN and CHAT_ID:
        ...
        try:
            ...
            print(f"\U0001F4E4 Telegram response: {response.status_code} - {response.text}")
...
def print_public_ip():
    try:
        ...
        print(f"🌍 Public IP used by Render: {ip}")
        send_telegram_alert(f"🌍 Render is using IP: {ip}")
    except Exception as e:
...
def run_whale_trap_worker():
    ...
    print_public_ip()
    send_telegram_alert("✅ Whale Trap Worker has started and is running...")
    while True:
